...
from ..helper.generate_code import generate_random_code
...
class Send_Email():
    ...
    def config_email(self, id_email, data, url, type='text'):
        ...
        self.select.exe_select("SELECT content_html, content_text, subject, type FROM email_data WHERE id = %s", f'{{"id": "{self.id_email}"}}', True)
        # self.select.exe_select("SELECT content_html, content_text, subject, type FROM email_data WHERE id = %s", f'{{"phone": "{data}", "LIMIT": "1"}}')
