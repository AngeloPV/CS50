...
class Update(Conn):
    ...
    def exe_update(self, data, table_name, data_where, close_conn=False, operator=None):
    ...
    def Repalce(self):
        ...
        if isinstance(self.data_where, dict):
            ...
            values_data_where = ", ".join(values_data_where.values())


        elif isinstance(self.data_where, str):
            values_data_where = self.data_where

        self.sql_update = f"UPDATE {self.table_name} SET {values_data} WHERE {self.data_where}"
    ...
    def update_table(self): 
        try:
            ...
            cursor.execute(self.sql_update, data_values + data_where)
