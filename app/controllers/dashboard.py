...
from controllers.get_crypto_data import crypto_data, do_update_database, get_crypto_buy, get_total_spent
...
def create_plot(crypto_name):
    do_update_database()
    if crypto_name == 'bitcoin':
        data = {
            'Period': ['6 months', '5 months', '4 months', '3 months', '2 months', '1 month', '7 days', '1 day', 'Now'],
            'Price': [
                crypto_data["bitcoin"]["price_6_months_ago"],
                crypto_data["bitcoin"]["price_5_months_ago"],
                crypto_data["bitcoin"]["price_4_months_ago"],
                crypto_data["bitcoin"]["price_3_months_ago"],
                crypto_data["bitcoin"]["price_2_months_ago"],
                crypto_data["bitcoin"]["price_1_month_ago"],
                crypto_data["bitcoin"]["price_7_days_ago"],
                crypto_data["bitcoin"]["price_1_day_ago"],
                crypto_data["bitcoin"]["current_price"]
            ]
        }
        ...
        data = {
            'Period': ['6 months', '5 months', '4 months', '3 months', '2 months', '1 month', '7 days', '1 day', 'Now'],
            'Price': [
                crypto_data["ethereum"]["price_6_months_ago"],
                crypto_data["ethereum"]["price_5_months_ago"],
                crypto_data["ethereum"]["price_4_months_ago"],
                crypto_data["ethereum"]["price_3_months_ago"],
                crypto_data["ethereum"]["price_2_months_ago"],
                crypto_data["ethereum"]["price_1_month_ago"],
                crypto_data["ethereum"]["price_7_days_ago"],
                crypto_data["ethereum"]["price_1_day_ago"],
                crypto_data["ethereum"]["current_price"]
            ]
        }
    else:
...
def create_plot_history(crypto_id, user_id):
    data = get_crypto_buy(crypto_id, user_id)
    ...
    df_grouped = clean_data(df_grouped)
...
def create_plot_spent(time, user_id):
    total_bitcoin = 0
    total_ethereum = 0

    data = get_total_spent(user_id, time)

    # Verificar e limpar os dados antes de usá-los
    cleaned_data = [(crypto_id, amount) for crypto_id, amount in data if not pd.isna(amount) and amount > 0]

    # Soma os valores para cada criptomoeda
    ...
    # Verificar se ambos os valores não são zero
    if total_bitcoin == 0 and total_ethereum == 0:
        # Criar um gráfico de placeholder
        fig, ax = plt.subplots(figsize=(7, 8))
        ...
        colors = ['#FF6F3C', '#1B263B']  # Laranja para Bitcoin, azul escuro para Ethereum

        # Criação do gráfico de pizza
        fig, ax = plt.subplots(figsize=(7, 8))

        # Exibição dos valores absolutos no gráfico de pizza
        wedges, texts, autotexts = ax.pie(valores, labels=labels, colors=colors, 
                                          autopct=lambda pct: '', startangle=90)
        ax.axis('equal')  # Para garantir que o gráfico de pizza seja um círculo

        # Ajuste da fonte
        for text in texts:
            text.set_fontsize(20)
            text.set_color('#F8F9FA')  # Cor da fonte para #F8F9FA

        for autotext in autotexts:
            autotext.set_fontsize(20)
            autotext.set_color('#F8F9FA')  # Cor da fonte para #F8F9FA
